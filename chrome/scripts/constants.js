            ...
            detectionFunctions: [
                function (node, nodeOld) {
                    ...
                    if (nodeOld && node.innerText != nodeOld.innerText) {
                         ...
                         * @constant
                         */
                        const reg = /(?:\d{1,2}\s*:\s*){1,3}\d{1,2}|(?:\d{1,2}\s*(?:days?|hours?|minutes?|seconds?|दिनों?|घंटे?|मिनट?|सेकंड?|[a-zA-Z]{1,3}\.?)(?:\s*(?:and|und|और))?\s*){2,4}/gi;
        ...
        },
        {
            ...
            detectionFunctions: [
                function (node, nodeOld) {
                    ...
                    //          "99% claimed"
                    return /\d+\s*(?:\%|pieces?|pcs\.?|pc\.?|ct\.?|items?)?\s*(?:available|sold|claimed|redeemed|off)|(?:last|final|off)\s*(?:article|item)/i.test(node.innerText);
                },
                function (node, nodeOld) {
                    // Return true if a match is found in the current text of the element,
